...
for seg_duration in seg_durations:
    ...
    filename1 = (
        PATH_cce1_processed
        / f"cce1_decor_scale_density_hrly_mooring_{option_interannual}_{seg_proc}_"
    ...
    with Dataset(filename1, "r") as nc:
        ...
        decor_scale1.append(nc.variables["decor_scale"][:])
        decor_scale1_stdm.append(nc.variables["decor_scale_stdm"][:])
    ...
    filename2 = (
        PATH_cce2_processed
        / f"cce2_decor_scale_density_hrly_mooring_{option_interannual}_{seg_proc}_"
            f"seg_duration_{seg_duration}mo.nc"
    )
    ...
    with Dataset(filename2, "r") as nc:
        ...
        # Append decorrelation scale to list
        decor_scale2.append(nc.variables["decor_scale"][:])
        decor_scale2_stdm.append(nc.variables["decor_scale_stdm"][:])
...
decor_scale1 = np.ma.stack(decor_scale1, axis=0)
decor_scale1_stdm = np.ma.stack(decor_scale1_stdm, axis=0)
decor_scale2 = np.ma.stack(decor_scale2, axis=0)
decor_scale2_stdm = np.ma.stack(decor_scale2_stdm, axis=0)
...
depth1_pos = np.abs(depth1) 
depth2_pos = np.abs(depth2) 
...
for isite, ax in enumerate(axes):
    ...
    for j, seg_duration in enumerate(seg_durations):
        ...
        if isite == 0: 
            Lt = decor_scale1[j,:]
            Lt_stdm = decor_scale1_stdm[j,:]
            depth_pos = depth1_pos
        else: 
            Lt = decor_scale2[j,:]
            Lt_stdm = decor_scale2_stdm[j,:]
            depth_pos = depth2_pos
